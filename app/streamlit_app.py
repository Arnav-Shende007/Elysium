...
Usage:
    Requires prior Colab authentication or GOOGLE_APPLICATION_CREDENTIALS env var.
...
# ──────────────────────────────────────────────
st.set_page_config(
    page_title="Elysium AI — Risk Intelligence",
    page_icon="🔍",
    layout="wide",
...
# HEADER
# ──────────────────────────────────────────────
st.markdown('<p class="hero-title">🔍 Elysium AI</p>', unsafe_allow_html=True)
st.markdown(
...
try:
    ...
    st.warning(
        f"⚠️ Dashboard data unavailable. Ensure BigQuery table `elysium.transactions_enriched` exists.\n\nError: {e}"
    )
    ...
    with st.chat_message("assistant"):
        with st.spinner("Analyzing..."):
            try:
                ...
                from model_router import ask_elysium, classify_query

                answer, model_used = ask_elysium(prompt)
...
# ──────────────────────────────────────────────
with st.sidebar:
    st.markdown("### ⚙️ About Elysium AI")
    st.markdown(
        """
    **Elysium AI** is a real-time financial risk intelligence
    platform powered by:
    ...
    **Model Routing:**
    ...
    st.markdown("---")
    st.caption("© 2025 Elysium AI • Powered by Google Cloud & NVIDIA")
